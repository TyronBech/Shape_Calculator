...
public class Circle{
	...
	public static void circle(JFrame frame, JPanel panel){
		...
		submit_button.addActionListener(e -> compute_circle(radius.getText()));
	...
	public static void oval(JFrame frame, JPanel panel){
		...
		submit_button.addActionListener(e -> compute_oval(a.getText(), b.getText()));
		frame.setVisible(true);
	}
	private static void compute_circle(String radius) {
		try {
			if(radius.isEmpty()) {
				throw new IOException();
			} else {
				double parsedRadius = Double.parseDouble(radius);
				if(parsedRadius <= 0) {
					throw new ArithmeticException();
				} else {
					Parent.area = Parent.PI * Math.pow(parsedRadius, 2);
	...
	private static void compute_oval(String a, String b) {
		try {
			if(a.isEmpty() || b.isEmpty()) {
				...
				if(parsed_a <= 0 || parsed_b <= 0) {
					...
					Parent.area = Parent.PI * parsed_a * parsed_b;
					DecimalFormat format_result = new DecimalFormat("0.00");
					String strResult = format_result.format(Parent.area);
					Output(strResult);
				}
